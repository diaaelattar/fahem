...
for (const id of ids) {
  ...
  if (!q) { console.log(`❓ لم يُعثر على ID: ${id}`); continue; }

  const fixedAnswer = 'صح';
  const fixedOptions = Array.isArray(q.options)
    ? q.options.map(o => {
        const v = (o || '').trim();
        if (['صواب', 'صح', 'صحيح', 'true', 'True', 'TRUE'].includes(v)) return 'صح';
        if (['خطأ', 'خاطئة', 'خاطئ', 'false', 'False', 'FALSE', 'ب', 'خ'].includes(v)) return 'خطأ';
        return o;
      })
    : q.options;
  ...
  if (error) {
    ...
    if (JSON.stringify(q.options) !== JSON.stringify(fixedOptions))
      console.log(`   الخيارات: ${JSON.stringify(q.options)} → ${JSON.stringify(fixedOptions)}`);
    success++;
  }
}
