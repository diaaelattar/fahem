...
const TYPE_LABELS = { mcq: 'اختيار من متعدد', true_false: 'صح / خطأ', fill_blank: 'ملء فراغ' }
const DIFF_LABELS = { easy: 'سهل', medium: 'متوسط', hard: 'صعب' }
const DIFF_COLORS = { easy: 'badge-easy', medium: 'badge-medium', hard: 'badge-hard' }
const TYPE_COLORS = { mcq: 'badge-mcq', true_false: 'badge-true-false', fill_blank: 'badge-fill-blank' }
...
export function QuestionPreviewGrid({
  ...
  const [filter, setFilter] = useState<'all' | 'mcq' | 'true_false' | 'fill_blank'>('all')
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white rounded-2xl border border-border p-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-1 bg-muted rounded-xl p-1">
            {(['all', 'mcq', 'true_false', 'fill_blank'] as const).map(f => (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {q.editing ? (
                    ...
                    <p className="text-base font-medium mb-3 leading-relaxed">{q.question_text}</p>
                  )}
                    ...
                    <div className="grid sm:grid-cols-2 gap-2 mb-3">
                      {q.options.map((opt, i) => (
                        <div key={i}
                          ...
                          {String.fromCharCode(0x200F)}{['أ', 'ب', 'ج', 'د'][i]}. {opt}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {q.type === 'fill_blank' && (
                    <div className="text-sm bg-green-50 border border-green-200 rounded-lg px-3 py-2 mb-3">
                      <span className="font-medium text-green-800">الإجابة: </span>
