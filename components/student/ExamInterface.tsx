...
export function ExamInterface({
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Override is_passed locally inside ExamInterface so UI matches percentage strictly (e.g. >= 50%)
          ...
          <div className="grid grid-cols-2 gap-4">
            {['صح', 'خطأ'].map(opt => (
              <button key={opt} onClick={() => handleAnswer(opt)}
                className={`answer-option flex-col justify-center py-8 ${answers[currentQ.id] === opt ? 'selected' : ''}`}>
