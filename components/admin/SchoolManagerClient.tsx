...
export function SchoolManagerClient({ initialSchools }: SchoolManagerClientProps) {
  ...
  const handleToggleActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      setSchools(
        schools.map((s) => (s.id === id ? { ...s, is_active: !currentStatus } : s))
      )
    } catch (err: any) {
      alert('فشل تعديل حالة تفعيل المدرسة: ' + err.message)
    }
  }
  ...
  const handleUpdatePlan = async (id: string, plan: string) => {
    try {
      ...
      setSchools(
        schools.map((s) => (s.id === id ? { ...s, subscription_plan: plan } : s))
      )
    } catch (err: any) {
      alert('فشل تحديث باقة الاشتراك: ' + err.message)
    }
  }
        ...
        <div className="overflow-x-auto">
          {filteredSchools.length > 0 ? (
            <table className="w-full text-right text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 text-xs font-bold bg-slate-50">
                  <th className="p-4 font-semibold">اسم المدرسة</th>
                  <th className="p-4 font-semibold">رابط المعرّف (Slug)</th>
                  <th className="p-4 font-semibold">نوع المرحلة</th>
                  <th className="p-4 font-semibold text-center">الباقة الحالية</th>
                  <th className="p-4 font-semibold text-center">الحالة</th>
                  <th className="p-4 font-semibold text-left">التفعيل</th>
                </tr>
              </thead>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              <School className="h-12 w-12 text-slate-200 mb-3" />
          ...
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl space-y-6 relative overflow-y-auto max-h-[90vh] animate-in fade-in zoom-in-95 duration-200" dir="rtl">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-2 text-slate-800">
                ...
                <h3 className="text-lg font-bold">تسجيل مدرسة جديدة بالمنصة</h3>
              </div>
              <button
