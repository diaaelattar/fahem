  ...
  try {
    if (!PAYMOB_API_KEY || !PAYMOB_INTEGRATION_ID || !PAYMOB_IFRAME_ID) {
      ...
      return NextResponse.json(
        { error: 'بوابات الدفع غير مفعلة حالياً' },
        { status: 500 }
      )
    }
      ...
      'https://accept.paymob.com/api/acceptance/payment_keys',
      {
        ...
        body: JSON.stringify({
          ...
          billing_data: {
            ...
            country: 'EG',
            last_name:
              profile?.full_name?.split(' ').slice(1).join(' ') || 'Name',
            state: 'NA',
          },
          currency: 'EGP',
    ...
    return NextResponse.json(
      { error: 'حدث خطأ غير متوقع أثناء الاتصال ببوابة الدفع' },
      { status: 500 }
    )
  }
}
