...
export async function POST(
  ...
  try {
    ...
    const challengerDone = isChallenger ? true : !!challenge.challenger_answers && Object.keys(challenge.challenger_answers).length > 0
    const opponentDone = !isChallenger ? true : !!challenge.opponent_answers && Object.keys(challenge.opponent_answers).length > 0
    ...
    if (challengerDone && opponentDone) {
      ...
      const winnerId = cScore > oScore ? challenge.challenger_id
        : oScore > cScore ? challenge.opponent_id
        : null // draw
