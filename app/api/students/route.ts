  ...
  try {
    ...
    if (profile?.role !== 'admin') {
      return NextResponse.json(
        { error: 'صلاحيات غير كافية — فقط المديرون يمكنهم إضافة طلاب' },
        { status: 403 }
      )
    }
    ...
    if (!email || !full_name || !password || !grade_id) {
      return NextResponse.json(
        {
          error:
            'البيانات المطلوبة: البريد الإلكتروني، الاسم، كلمة المرور، الصف',
        },
        { status: 400 }
      )
    }
    ...
    if (studentError) {
      ...
      return NextResponse.json(
        { error: `خطأ في إنشاء سجل الطالب: ${studentError.message}` },
        { status: 500 }
      )
    }
