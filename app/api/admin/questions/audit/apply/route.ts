...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      typeof suggestions.question_text === 'string' &&
      suggestions.question_text.trim()
    )
      update.question_text = suggestions.question_text.trim()

    if (
      typeof suggestions.correct_answer === 'string' &&
      suggestions.correct_answer.trim()
    )
      update.correct_answer = suggestions.correct_answer.trim()

    if (
      typeof suggestions.explanation === 'string' &&
      suggestions.explanation.trim()
    )
    ...
    return NextResponse.json(
      { error: error.message || 'حدث خطأ أثناء حفظ التعديلات' },
      { status: 500 }
    )
  }
}
