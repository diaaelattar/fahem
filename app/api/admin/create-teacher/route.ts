...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!email || !fullName || !password) {
      return NextResponse.json({ error: 'البيانات المطلوبة: البريد الإلكتروني، الاسم، كلمة المرور' }, { status: 400 })
    }
    ...
    if (teacherError) {
      ...
      return NextResponse.json({ error: `خطأ في إنشاء سجل المعلم: ${teacherError.message}` }, { status: 500 })
    }
