...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!amount || !reason) return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
    ...
    return NextResponse.json({ success: true, earned: amount, total: newTotal, level: newLevel })
  } catch (err: any) {
