...
export default async function TakeExamPage({ params, searchParams }: Props) {
  ...
  if (!attempt) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <p className="text-muted-foreground mb-6">لم يتم العثور على هذه المحاولة أو انتهت صلاحيتها</p>
  ...
  const { data: exam } = await supabase
    .from('exams')
    .select('id, title, duration_minutes, total_points, passing_score, show_results_immediately, instructions, subjects(name_ar, icon), grades(name_ar)')
    .eq('id', params.id)
    .single()

  if (!exam) notFound()

  // جلب أسئلة الاختبار (بدون الإجابات الصحيحة)
  ...
  // استعادة الإجابات المحفوظة مسبقاً إن وجدت
