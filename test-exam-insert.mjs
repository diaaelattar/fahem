...
async function testInsertExam() {
   ...
   const { data: subject } = await supabaseAdmin.from('subjects').select('id').limit(1).single();
   const { data: grade } = await supabaseAdmin.from('grades').select('id').limit(1).single();
